...
export function Sidebar({ isMobileOpen, closeMobile }: Props) {
  ...
  const userNavItems = user?.isSuspended ? [
    ...
    { label: "Help / Complaints", path: "/dashboard/complaints", icon: AlertTriangle },
    ...
    <>
      ...
      <aside className={cn(
        ...
        <div className="p-6 flex items-center justify-between border-b border-gray-100 flex-shrink-0">
           ...
           {isMobileOpen && (
             <button type="button" onClick={closeMobile} className="md:hidden text-gray-500 hover:bg-gray-100 p-1.5 rounded-md">
               <X className="size-5" />
             </button>
           )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto w-full">
          {inAdminRoute ? (
            <div className="p-4 flex flex-col gap-1">
               <p className="px-4 text-xs font-bold text-red-500 uppercase tracking-wider mb-2 flex items-center gap-2"><ShieldCheck className="size-3.5"/> Admin Panel</p>
               ...
               <div className="mt-8 border-t border-gray-100 pt-4">
                 <NavLink to="/dashboard" className="flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-gray-500 hover:bg-gray-50 transition-all text-sm">
                   <ArrowLeft className="size-5" /> Exit Admin
                 </NavLink>
               </div>
            </div>
          ) : (
              ...
              {isAdmin && (
                <div className="mt-8 border-t border-gray-100 pt-4">
                   <NavLink to="/admin" className="flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-red-500 bg-red-50 hover:bg-red-100 transition-all text-sm shadow-sm">
                     <ShieldCheck className="size-5" /> Go to Admin Panel
                   </NavLink>
                </div>
              )}
            </div>
        ...
        <div className="p-4 border-t border-gray-100 flex-shrink-0">
          <button type="button" onClick={logout} className="flex flex-row items-center gap-3 px-4 py-3 w-full rounded-xl font-medium text-gray-600 hover:bg-red-50 hover:text-red-600 transition-all duration-200 text-sm">

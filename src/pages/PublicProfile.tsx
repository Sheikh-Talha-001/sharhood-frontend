    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      <div className="bg-white rounded-4xl border border-gray-100 overflow-hidden shadow-sm">
        <div className="h-32 md:h-48 bg-gradient-to-r from-gray-100 to-gray-200 w-full relative">
           ...
           >
             <Flag className="size-3.5" /> Report User
           </button>
        </div>
        <div className="px-8 md:px-12 pb-10 relative">
          {/* Avatar */}
          <div className="size-24 md:size-32 bg-brand-black rounded-2xl border-4 border-white shadow-xl flex items-center justify-center -mt-12 md:-mt-16 mb-4 relative z-10 overflow-hidden">
             {user.avatar ? (
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                {user.name}
                {isPartner && <Store className="size-6 text-brand-yellow" title="Approved Partner" />}
              </h1>
              
              <div className="flex flex-wrap items-center gap-4 mt-3 text-sm font-medium text-gray-600">
                 {user.neighborhood && (
                   <span className="flex items-center gap-1.5"><MapPin className="size-4 text-gray-400"/> {user.neighborhood}</span>
                 )}
                 <span className="flex items-center gap-1.5"><Calendar className="size-4 text-gray-400"/> Member since {new Date(user.createdAt).getFullYear()}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {isVerified && (
                <div className="bg-green-50 text-green-600 px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-bold shadow-sm">
                  <ShieldCheck className="size-5" /> Verified ID
                </div>
              )}
            </div>
          </div>

...
export function Verification() {
  ...
  if (isLoading !== false) return <div className="py-20"><LoadingSpinner /></div>;

  return (
    <div className="max-w-3xl mx-auto">
      <div className="text-center mb-10">
        <div className="size-16 bg-blue-50 text-blue-500 rounded-2xl flex items-center justify-center mx-auto mb-6">
         ...
         {status === "unsubmitted" && (
           <div className="flex border-b border-gray-100 bg-gray-50">
              <div className="flex-1 p-4 text-center border-r border-gray-100">
                 <div className={`size-8 rounded-full flex items-center justify-center mx-auto mb-2 text-sm font-bold ${step === 1 ? 'bg-brand-black text-white' : 'bg-green-100 text-green-600'}`}>1</div>
                 <span className="text-sm font-bold text-gray-900">Basic Info</span>
              </div>
              <div className="flex-1 p-4 text-center border-r border-gray-100">
                 <div className={`size-8 rounded-full flex items-center justify-center mx-auto mb-2 text-sm font-bold ${step === 2 ? 'bg-brand-black text-white' : 'bg-gray-200 text-gray-500'}`}>2</div>
         ...
         <div className="p-8 md:p-12">
            ...
            {status === "unsubmitted" && step === 1 && (
              <div className="space-y-6">
                 ...
                 <div className="pt-4">
                    <button type="button" onClick={() => setStep(2)} className="w-full bg-brand-black text-white font-bold py-4 rounded-full hover:bg-brand-yellow hover:text-brand-black transition-all">Next Step: Document Upload</button>
                 </div>
              </div>
            )}
            ...
            {status === "unsubmitted" && step === 2 && (
              <div className="space-y-6">
                 ...
                 <div className="pt-4 flex gap-4">
                    <button type="button" onClick={() => setStep(1)} className="px-6 py-4 rounded-full bg-gray-100 font-bold hover:bg-gray-200 transition-colors">Back</button>
                    <button type="button" onClick={handleSubmit} className="flex-1 bg-brand-black text-white font-bold py-4 rounded-full hover:bg-brand-yellow hover:text-brand-black transition-all">Submit Verification</button>
                 </div>
              </div>
            )}
            ...
            {status === "rejected" && (
              <div className="text-center py-8">
                 ...
                 <button type="button" onClick={() => {setStatus("unsubmitted"); setStep(1);}} className="bg-brand-black text-white px-8 py-3 rounded-full font-bold hover:bg-brand-yellow hover:text-black">Try Again</button>
              </div>
            )}
         </div>
      </div>
      ...
      <p className="text-center text-xs font-bold text-gray-400 uppercase tracking-wider mt-8 flex items-center justify-center gap-2">
        <Shield className="size-4" /> Your data is encrypted and stored securely.
      </p>
    </div>
  );
}
